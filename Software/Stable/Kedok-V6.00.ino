...
To Do:
 ...
 Menu of adavanced user is a bit strange in usage.
...
void PlayHelp(byte Option) {
  ...
  switch(Option) {
    ...
    case 10: PlaySound(HelpSetPitchStepMP3,12,0); break; 
...
byte MainMenuSelection() {
    ...
    while(!Esc) {
      switch (OptionNr) {
          ...
          case 10: ShowOLED("[Pitch step]", 0,4,1); PlaySound(SetPitchStepMP3,8,0); break;
...
void loop() {
 ...
 //--------Kernel part-------- 
  if (Reading < MinValue) {
     ...
     if (PitchStep) { if (AudioTone > (HighTone-PitchStepValue)) AudioTone= AudioTone+PitchStepValue; }//PitchStep around the 8 on the target card. //seems to be buggy if you higher the tone. Benjamin 25/10/2017
     PlayTone(AudioTone,0);
